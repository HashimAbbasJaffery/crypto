...
class Setting extends Model
{
    ...
    public static function active_currency($output = '')
    {
        ...
        foreach ($all_currency as $item) {
            $pmc_active = Setting::getValue('pmc_active_' . $item);
            if ($pmc_active) {
                if (Setting::getValue('pm_exchange_method') == 'automatic') {
                    $currencies[$item] = Setting::getValue('pmc_auto_rate_' . strtolower($item));
                }
                else {
                    $currencies[$item] = Setting::getValue('pmc_rate_' . $item);
                }
            }
            // return $result;
            // dd($result);
